...
class fli_FlightActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val registerButton = Button(this).apply {
            ...
            setOnClickListener { registerFlight() }
        }

        val viewFlightsButton = Button(this).apply {
            text = "Vuelos"
            setOnClickListener {
                val intent = Intent(this@fli_FlightActivity, fli_FlightMenuActivity::class.java)
    ...
    private fun registerFlight() {
        ...
        // Simple date/time validation to ensure they are selected
        if (fli_date == "Seleccionar Fecha" || fli_time == "Seleccionar Hora") {
            Toast.makeText(this, "Por favor seleccione fecha y hora.", Toast.LENGTH_SHORT).show()
            return
        }

        val flight = fli_Flight(fli_id, fli_name, fli_lastName, fli_originCountry, fli_destinationCountry, fli_flightNumber, "$fli_date $fli_time")
        fli_flightController.registerFlight(flight)

        Toast.makeText(this, "Vuelo registrado exitosamente.", Toast.LENGTH_SHORT).show()
    }
}
